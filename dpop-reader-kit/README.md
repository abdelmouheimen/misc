...
Ce dépôt permet de **voir le mécanisme fonctionner, puis de l'attaquer** : un backend minimal
(Kotlin / Spring Boot), un générateur de preuves DPoP (Python) et une série de scénarios
d'attaque rejouables.

> ⚠️ Lab pédagogique, volontairement simplifié et non sécurisé. Lire [SECURITY.md](SECURITY.md).

## Prérequis

- **JDK 21** et **Maven** (ou **Docker**, au choix)
- **Python 3.9+** avec `cryptography` : `pip install cryptography`
- `bash` et `curl` pour les scripts d'attaque (Git Bash sous Windows)

## Démarrage en trois commandes

```bash
# 1. Lancer le backend (DPoP exigé, port 8099)
cd backend && mvn spring-boot:run
#    …ou sans Maven :   docker compose up --build

# 2. Dans un autre terminal : première attaque, un jeton volé ne suffit pas
...
Ouvrez aussi la **page de test navigateur** : <http://localhost:8099/>. Elle génère une clé
non exportable via Web Crypto, signe de vraies preuves, et démontre que la clé privée ne peut
pas être extraite de JavaScript.
...
| `backend/…/static/index.html` | Page de test Web Crypto |
| `tools/dpop_proof.py` | Générateur de preuves DPoP, valides ou volontairement invalides |
| `attacks/` | Huit scénarios rejouables (voir ci-dessous) |
